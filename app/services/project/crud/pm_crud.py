...
class ProjectMCrud:
    ...
    @staticmethod
    async def query_pm_with_id(project_id: int) -> List[int]:
        async with async_session() as session:
            smtm = """
                SELECT pm.user_id FROM project_member pm
                WHERE pm.project_id = :project_id AND pm.deleted_at = 0
            """
            execute_smtm = await session.execute(text(smtm), {"project_id": project_id})
            result = execute_smtm.scalars().all()
            return list(result)
