...
class NewSuffixServices:
    ...
    async def execute_method_delay(self, delay: int, log_type: str):
        ...
        self.log[log_type].append(f"[{t}]: [延迟执行] -> {delay/1000}ms")
    ...
    async def _execute(self, model: Union[SuffixModel, SchemaCaseSuffix], log_type: str):
        if model.enable:
            ...
            await redis_client.set_case_log(user_id=self.user_id, value=self.log, case_id=self.case_id)
    ...
    async def execute_case_prefix(self, is_prefix: bool, temp_prefix: List[SchemaCaseSuffix] = None):
        if is_prefix:
