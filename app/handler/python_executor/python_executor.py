...
class PythonExecutor:
    ...
    def validate_module(script_text: str):
        ...
        for result in valid_check:
            for module_name in result:
                if module_name and module_name not in PythonExecutor.white_module:
    ...
    async def python_executor(get_var: str, script_text: str, check_module=True):
        ...
        # 模块导入检查
        if check_module:
            PythonExecutor.validate_module(script_text)
        ...
        try:
            ...
            await asyncio.wait_for(
                asyncio.to_thread(exec, compiled_code, PythonExecutor.temp_namespace),
                timeout=5,
            )
            result = eval(get_var, PythonExecutor.temp_namespace)
            PythonExecutor.temp_namespace.pop(get_var)
