...
async def custom_exception_handler(request: Request, exc: CustomException):
    log.error(f"请求异常: {json.dumps(exc.error_message, ensure_ascii=False, indent=2)}")
    return JSONResponse(
...
async def validation_response_exp_handler(request, exc: ResponseValidationError):
    log.error(f"响应校验异常: {json.dumps(exc.errors(), ensure_ascii=False, indent=2)}")
    return JSONResponse(
